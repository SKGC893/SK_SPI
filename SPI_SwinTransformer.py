...
class DropPath(nn.Module):
    ...
    def forward(self, x):
        ...
        output = x.div(keep_prob) * random_tensor       # 如果为0则路径被丢弃，如果为1则放大1/keep_prob倍，防止整体期望降低
        return output
...
class PatchEmbed(nn.Module):
    '''这里就是ViT继承过来的重要部分了
    为了解决单个像素作为token计算量过大的问题，ViT使用patch将图像分割成许多图像块
    每个图像块展平线性投影成一个特征向量，这个特征向量就被视为一个token'''
    def __init__(self, patch_size = 4, in_c = 3, embed_dim = 64, norm_layer = None):
        super().__init__()
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, dim, norm_layer = nn.LayerNorm):
        ...
        '''这一步降低了特征图的空间分辨率（从H * W变为H/2 * W/2，即Patch数量减少了四倍）
        但是我们是将原本的4个patch(4C)拼接并通过线性压缩层变为2C
        对于原本的每个patch，通道数是C，所以对于单个patch实际上是分辨率降低同时通道翻倍'''
    ...
    def forward(self, x, H, W):
        ...
        # PatchMerging的核心逻辑，将原始特征图上相邻的2*2的patch提取出来并沿着通道维度拼接
        x0 = x[:, 0::2, 0::2, :]        # 左上角
        x1 = x[:, 1::2, 0::2, :]        # 左下角
        x2 = x[:, 0::2, 1::2, :]        # 右上角
        x3 = x[:, 1::2, 1::2, :]        # 右下角
        x = torch.cat([x0, x1, x2, x3], -1)     # 沿着最后一个维度（通道维度）拼接
        x = x.view(B, -1, 4 * C)        # [B, H_padded/2 * W_padded/2, C]
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias = True, attn_drop = 0., proj_drop = 0., ):
        ...
        '''计算相对位置索引
        都是从0开始递增到arange内部的值-1，试着想象一下坐标轴的样子'''
        ...
        # 广播相减得到所有patch的相对坐标，relative_coords[0,]表示行，relative_coords[1,]表示列
        ...
        # 注册缓冲区，初始化的时候就会计算好，但是不会随着模型训练在反向传播种更新
    ...
    def forward(self, x, mask = None):
        ...
        qkv = self.qkv(x).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)     # 提取qkv
...
class SPISwinTransformer(nn.Module):
    def __init__(self, sampling_times, img_size,    # 这两个是最前面卷积的参数
        ...
        self.avgpool = nn.AdaptiveAvePool1d(1)
